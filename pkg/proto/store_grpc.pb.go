...
type KVStoreClient interface {
	...
	// another.
	Sync(ctx context.Context, opts ...grpc.CallOption) (KVStore_SyncClient, error)
}
...
func (c *kVStoreClient) Identify(ctx context.Context, in *IdentityRequest, opts ...grpc.CallOption) (*IdentityResponse, error) {
	out := new(IdentityResponse)
	err := c.cc.Invoke(ctx, "/kv.KVStore/Identify", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *kVStoreClient) Sync(ctx context.Context, opts ...grpc.CallOption) (KVStore_SyncClient, error) {
	stream, err := c.cc.NewStream(ctx, &KVStore_ServiceDesc.Streams[1], "/kv.KVStore/Sync", opts...)
	if err != nil {
		return nil, err
	}
	x := &kVStoreSyncClient{stream}
	return x, nil
}

type KVStore_SyncClient interface {
	Send(*SyncMessage) error
	CloseAndRecv() (*Empty, error)
	grpc.ClientStream
}

type kVStoreSyncClient struct {
	grpc.ClientStream
}

func (x *kVStoreSyncClient) Send(m *SyncMessage) error {
	return x.ClientStream.SendMsg(m)
}

func (x *kVStoreSyncClient) CloseAndRecv() (*Empty, error) {
	if err := x.ClientStream.CloseSend(); err != nil {
		return nil, err
	}
	m := new(Empty)
...
func (UnimplementedKVStoreServer) Identify(context.Context, *IdentityRequest) (*IdentityResponse, error) {
	return nil, status.Errorf(codes.Unimplemented, "method Identify not implemented")
}
func (UnimplementedKVStoreServer) Sync(KVStore_SyncServer) error {
...
func _KVStore_Identify_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(IdentityRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(KVStoreServer).Identify(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/kv.KVStore/Identify",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(KVStoreServer).Identify(ctx, req.(*IdentityRequest))
	}
	return interceptor(ctx, in, info, handler)
}

func _KVStore_Sync_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(KVStoreServer).Sync(&kVStoreSyncServer{stream})
}

type KVStore_SyncServer interface {
	SendAndClose(*Empty) error
	Recv() (*SyncMessage, error)
	grpc.ServerStream
}

type kVStoreSyncServer struct {
	grpc.ServerStream
}

func (x *kVStoreSyncServer) SendAndClose(m *Empty) error {
	return x.ServerStream.SendMsg(m)
}

func (x *kVStoreSyncServer) Recv() (*SyncMessage, error) {
	m := new(SyncMessage)
...
var KVStore_ServiceDesc = grpc.ServiceDesc{
	...
	Methods: []grpc.MethodDesc{
		...
		{
			MethodName: "Identify",
			Handler:    _KVStore_Identify_Handler,
		},
	},
